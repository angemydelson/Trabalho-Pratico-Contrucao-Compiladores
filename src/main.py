# Copyright
# Part of this project has been cloned from project(CleissonVieira) in order to generate the 'AFND'
...
class Automato(object):  # carga do automato finito
    ...
    def carregaGramatica(self, textos):  # leitura das regras da Gramática Regular
        ...
        for linha in textos:  # percorre as linhas do texto de entrada
            ...
            for caractere in linha:  # percorre os caracteres da linha
                ...
                elif (re.match('\|\S<\S>', word) is not None or re.match('=\S<\S>',
                                                                         word) is not None):  # se palavra tem formato de uma transição
                    novaTransicao(self, word[1], word[3])  # add uma nova transição à regra ativa
                    word = ''

                elif (re.match('\|<\S>', word) is not None or re.match('=<\S>',
                                                                       word) is not None):  # se palavra tem formato de um nome de regra e está no meio da regra
    ...
    def carrega(self, arquivo):  # Insere no automato
        ...
        self.Estados.update(
            {len(self.Estados): {}})  # inicializa o estado inicial com: um inteiro para chave e um dicionário vazio para conteúdo
        ...
        if len(self.Estados[transicaoOriginal][self.EPSILON]) > 0:
            ...
            for producao in list(self.Estados[transicaoEpsilon]):
                if producao != self.EPSILON and len(self.Estados[transicaoEpsilon][producao]) > 0:
                    self.Estados[transicaoEpsilon][producao] = (
                        list(set(self.Estados[transicaoEpsilon][producao] + self.Estados[transicaoOriginal][producao])))
...
print("-======- Eliminar epsilon transições -=======-")
livreEpsilon.imprimir(automato)
...
class Determinizacao(Automato):
            ...
            for estado in self.Estados:  # percorre estado do automato
                for simbolo in sorted(self.Alfabeto):  # percorre símbolos do alfabeto
                    if len(self.Estados[estado][simbolo]) > 1:  # se qtd de transições por uma prod for > 1
                        producaoAgrupada = self.geraProducaoAgrupada(
                            self.Estados[estado][simbolo])  # gera uma prod agrupada para conhecer a origem da nova prod
    ...
    def determinizarProducao(self, producoes):  # determiniza x estado do automato
        ...
        if ((producaoAgrupada not in self.NovosEstados) or (self.NovosEstados[producaoAgrupada][
                                                                0] not in self.Estados)):  # se prod agrupada não existir nas novas prod ou nova regra não estiver no automato
            ...
            for i in producoes:  # percorre prod que contém indeterminação
                for j in sorted(self.Alfabeto):  # percorre conjunto de símbolos do alfabeto
                    if j in estadoTemporario:  # se símbolo j já estiver no estado temporário
                        ...
                        if (len(lista) > 1) and (
                                not self.existeProducaoAgrupada(lista)):  # se largura da lista for > 1 e não existe prod agrupada pra ela
                        ...
                        elif (len(producaoAtual) > 0) and (
                                self.existeNovoEstado(
                                    producaoAtual[0])):  # se não se essa prod não for nula e existe um estado para essa transição
            ...
            self.Estados.update(
                {self.NovosEstados[producaoAgrupada][0]: estadoTemporario});  # add o estado temporário ao dict de estados da classe
...
print('-=========- Determinizar o automato -==========-')
determinizado.imprimir(automato)
...
print('-========- O automato apos a remoção de Inalcancaveis -=========-')
semInalcancaveis.imprimirTela(automato)
    ...
    def removerMortos(self):
        ...
        for transicao in estados:
            for producao in estados[transicao]:
                ...
                estados[transicao][producao].chegouEstadoTerminal = self.adicionaAutomatoMinimizado(transicao, producao,
                                                                                                    estados[transicao][producao].producao)
